...
// Dynamically imported (no SSR) to prevent hydration mismatch on wallet state
...
function AuthPageContent() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus((err as Error).message || "Failed to send magic link");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleWalletLogin = async () => {
    ...
    setLoading(true);
    try {
      const nonceRes = await api("/api/v1/auth/wallet/nonce", {
        method: "POST",
        body: JSON.stringify({ wallet_address: publicKey.toBase58() }),
      }) as { message: string; nonce: string };

      const messageBytes = new TextEncoder().encode(nonceRes.message);
      const signatureBytes = await signMessage(messageBytes);
      const signature = bs58.encode(signatureBytes);

      const verifyRes = await api("/api/v1/auth/wallet/verify", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        <div className="glass-card p-10 md:p-14 rounded-[3.5rem] border-white/10 shadow-2xl relative">
          <div className="flex bg-white/5 p-2 rounded-2xl mb-12 border border-white/5">
            <button onClick={() => setMode("merchant")} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${mode === "merchant" ? "bg-primary text-primary-foreground shadow-lg" : "text-gray-500 hover:text-white"}`}>
              Merchant Portal
            </button>
            <button onClick={() => setMode("customer")} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${mode === "customer" ? "bg-primary text-primary-foreground shadow-lg" : "text-gray-500 hover:text-white"}`}>
              Customer App
            </button>
          </div>
          ...
          {mode === "merchant" ? (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex justify-center flex-col items-center gap-6">
                ...
                {connected && (
                  <Button type="button" className="w-full h-16 text-lg font-bold" onClick={handleWalletLogin} loading={loading} disabled={!publicKey || !signMessage || connecting}>
                    Sign In with Wallet →
                  </Button>
                )}
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-4">
                <Input type="email" placeholder="your@email.com" required value={email} onChange={(e) => setEmail(e.target.value)} className="h-16 bg-white/5 border-white/10 text-lg rounded-2xl" />
              ...
              {connected && (
                <div className="flex justify-center mt-4">
                  <Button type="button" className="w-full h-14 text-base font-bold" onClick={handleWalletLogin} loading={loading} disabled={!publicKey || !signMessage || connecting}>
          ...
          {status && <div className="mt-6 text-center text-xs text-gray-400">{status}</div>}

          <div className="mt-12 text-center">
            <p className="text-gray-600 text-[10px] uppercase tracking-widest font-bold">Powered by Optik Autonomous Infrastructure</p>
          </div>
        </div>
      </div>
    </div>
