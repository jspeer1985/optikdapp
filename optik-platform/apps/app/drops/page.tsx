      ...
      <div className="max-w-6xl mx-auto px-6 space-y-10">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {drops.map((drop) => (
              <Link key={drop.job_id} href={`/dapps/${drop.job_id}`} className="rounded-3xl border border-white/10 bg-white/5 p-6 hover:bg-white/10 transition-colors">
                ...
                <p className="text-sm text-gray-500 mt-3">{drop.dapp_url || 'View storefront'}</p>
              </Link>
            ))}
          </div>
        )}
